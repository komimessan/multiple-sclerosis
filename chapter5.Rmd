...
```{r  message=FALSE, warning=FALSE, include=FALSE}
## Conduct the wilcoxon test between HV and MS cohorts for Features that are stables by ICC>0.75

#### Get Features that are stable for the Healthy Volunteers
StableF_doml1 <- ICCven_func(icc.HV.MS.doml1,0.75)$HV
StableF_ndoml1 <- ICCven_func(icc.HV.MS.ndoml1,0.75)$HV
StableF_doml2 <- ICCven_func(icc.HV.MS.doml2,0.75)$HV
StableF_ndoml2 <- ICCven_func(icc.HV.MS.ndoml2,0.75)$HV
StableF_doml3 <- ICCven_func(icc.HV.MS.doml3,0.75)$HV
StableF_ndoml3 <- ICCven_func(icc.HV.MS.ndoml3,0.75)$HV
...
full_feat_train_Sdoml1 <- full_feature_train %>% 
  ...
  dplyr::select(-setdiff(paste("F",1:40,sep =""), ICCven_func(icc.HV.MS.doml1,0.75)$HV)) 
full_feat_train_Sndoml1 <- full_feature_train %>% 
  subset(difficulty_level=="1" & dominant_hand_use=="NO") %>% 
  dplyr::select(-setdiff(paste("F",1:40,sep =""), ICCven_func(icc.HV.MS.ndoml1,0.75)$HV)) 

## Difficulty Level 2
full_feat_train_Sdoml2 <- full_feature_train %>% 
  subset(difficulty_level=="2" & dominant_hand_use=="YES") %>% 
  dplyr::select(-setdiff(paste("F",1:40,sep =""), ICCven_func(icc.HV.MS.doml2,0.75)$HV)) 
full_feat_train_Sndoml2 <- full_feature_train %>% 
  subset(difficulty_level=="2" & dominant_hand_use=="NO") %>% 
  dplyr::select(-setdiff(paste("F",1:40,sep =""), ICCven_func(icc.HV.MS.ndoml2,0.75)$HV)) 

## Difficulty Level 3
full_feat_train_Sdoml3 <- full_feature_train %>% 
  subset(difficulty_level=="3" & dominant_hand_use=="YES") %>% 
  dplyr::select(-setdiff(paste("F",1:40,sep =""), ICCven_func(icc.HV.MS.doml3,0.75)$HV)) 
full_feat_train_Sndoml3 <- full_feature_train %>% 
  subset(difficulty_level=="3" & dominant_hand_use=="NO") %>% 
  dplyr::select(-setdiff(paste("F",1:40,sep =""), ICCven_func(icc.HV.MS.ndoml3,0.75)$HV)) 
...
data11 <- na.omit(data.frame(mat.feat_func.onel(full_feat_train_Sdoml1,cl_feature.HV.MS,"HV",0.001)$pat_data[,c("F5","F38")], mat.feat_func.onel(full_feat_train_Sdoml1,cl_feature.HV.MS,"HV",0.001)$cl_data[,c("C8","C21")]))
data12 <- na.omit(data.frame(mat.feat_func.onel(full_feat_train_Sdoml1,cl_feature.HV.MS,"MS",0.001)$pat_data[,c("F5","F38")], mat.feat_func.onel(full_feat_train_Sdoml1,cl_feature.HV.MS,"MS",0.001)$cl_data[,c("C8","C21")]))

PerformanceAnalytics::chart.Correlation(data11, histogram=TRUE, pch=19, method = "spearman")

data21 <- na.omit(data.frame(mat.feat_func.onel(full_feat_train_Sdoml2,cl_feature.HV.MS,"HV",0.001)$pat_data[,c("F6","F11","F26")], mat.feat_func.onel(full_feat_train_Sdoml2,cl_feature.HV.MS,"HV",0.001)$cl_data[,c("C14","C8")]))
data22 <- na.omit(data.frame(mat.feat_func.onel(full_feat_train_Sdoml2,cl_feature.HV.MS,"MS",0.001)$pat_data[,c("F6","F11","F26")], mat.feat_func.onel(full_feat_train_Sdoml2,cl_feature.HV.MS,"MS",0.001)$cl_data[,c("C14","C8")]))
