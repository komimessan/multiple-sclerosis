...
To later calculated the Intraclass correlation coefficient (ICC), We extract the longitudinal data (i.e. patient that took the test more than 5 times during the two years period). The ICC was only calculated for the Healthy Volunteers since we expect more stability in features in healthy cohorts as oppose to the MS cohorts.
...
Now we construct a function to calculate the p-value between the HV and MS. To calculate the Log Fold change (Log2FC), we used the following procedure: 
$$Log2FC = \frac{mean\{Log2(|Feature \quad in \quad HV|) + 1\}}{mean\{Log2(|Feature \quad in \quad MS|) + 1\}}$$
where group 1 and 2 could either be HV or MS and $|.|$ is the absolute value of the features.


Here we also calculated the wilcoxon test and also the effect size. Note that a significant p-value tells us that a treatment works, whereas an effect size tell us how much it works. According to Cohen et al., effect size of 0.1 < 0.3 is small, 0.3 < 0.5 is moderate, and >=0.5 is a large effect. See more in [Datanovia](https://www.datanovia.com/en/lessons/wilcoxon-test-in-r/#effect-size) or different ways to calculate it in [Datanovia2](https://www.datanovia.com/en/lessons/t-test-effect-size-using-cohens-d-measure/#:~:text=T%2Dtest%20conventional%20effect%20sizes,if%20it%20is%20statistically%20significant.)
...
Below we plotted the ICC for each features
...
Nice.Label2 <-theme(axis.text.x = element_text(face="bold", color="black", size=11),
         ...
         legend.box = "vertical", 
         legend.text = element_text(size=24),strip.text.x = element_text(face="bold",size=24), strip.text.y = element_text(face="bold",size=24)) 
##################
...
```
 ...
 We now extract the statistically significant features and calculte the Log2 Fold-Change 
...
icc.fc.doml1 <- data.frame(merge(icc.HV.doml1, fc.doml1, by = "Feature"), difficulty = "Difficulty Level 1")
icc.fc.doml2 <- data.frame(merge(icc.HV.doml2, fc.doml2, by = "Feature"), difficulty = "Difficulty Level 2")
icc.fc.doml3 <- data.frame(merge(icc.HV.doml3, fc.doml3, by = "Feature"), difficulty = "Difficulty Level 3")

icc.fc.ndoml1 <- data.frame(merge(icc.HV.ndoml1, fc.ndoml1, by = "Feature"), difficulty = "Difficulty Level 1")
icc.fc.ndoml2 <- data.frame(merge(icc.HV.ndoml2, fc.ndoml2, by = "Feature"), difficulty = "Difficulty Level 2")
icc.fc.ndoml3 <- data.frame(merge(icc.HV.ndoml3, fc.ndoml3, by = "Feature"), difficulty = "Difficulty Level 3")
...
#### Combine Fold-change and ICC of healthy volunteers data frame
icc.fc.doml1.tst <- data.frame(merge(icc.HV.doml1, fc.doml1.tst, by = "Feature") ,
                               difficulty="Difficulty Level 1") 
icc.fc.doml2.tst <- data.frame(merge(icc.HV.doml2, fc.doml2.tst, by = "Feature"),
                               difficulty="Difficulty Level 2")
icc.fc.doml3.tst <- data.frame(merge(icc.HV.doml3, fc.doml3.tst, by = "Feature"),
                               difficulty="Difficulty Level 3")

icc.fc.ndoml1.tst <- data.frame(merge(icc.HV.ndoml1, fc.ndoml1.tst, by = "Feature"),
                               difficulty="Difficulty Level 1")
icc.fc.ndoml2.tst <- data.frame(merge(icc.HV.ndoml2, fc.ndoml2.tst, by = "Feature"),
                               difficulty="Difficulty Level 2")
icc.fc.ndoml3.tst <- data.frame(merge(icc.HV.ndoml3, fc.ndoml3.tst, by = "Feature"),
                               difficulty="Difficulty Level 3")
...
No we plot the ICC vs the Log2 Fold Change for significant features. Note that FC is a ratio of one value to another. One of the values is used as the reference point. In our case, we used treatment / control where the control value is used as a reference point. Fold-change of 2 means treatment is twice the value of control.
...
ggplot(icc.fc.all, aes(x=FC, y=ICC, label = Label, color=Type)) +
  ...
  geom_text(size = 5, alpha=1.2, color="black") + xlim(2,3)+ylim(0.5,1.00)+
  labs(x = "", y ="") + 
...
mat.feat_func.onel <- function(pat_data,cl_data, diagnosis, alpha){
  ...
  corrplot(r_pat_cl, type="full", order="original", col = col_cor,
         p.mat = p_pat_cl, sig.level = 0.001, insig = "blank", diag = TRUE,
...
## Training set
## Difficulty Level 1
mat.feat_func.onel2(full_feat_train_doml1,cl_feature.HV.MS,"HV",0.05)
mat.feat_func.onel2(full_feat_train_doml1,cl_feature.HV.MS,"MS",0.05)
mat.feat_func.onel2(full_feat_train_ndoml1,cl_feature.HV.MS,"HV",0.05)
mat.feat_func.onel2(full_feat_train_ndoml1,cl_feature.HV.MS,"MS",0.05)

## Test set (HV are the same as in Traning)
## Difficulty Level 1
mat.feat_func.onel2(full_feat_test_doml1,cl_feature.HV.MS,"MS",0.05)
mat.feat_func.onel2(full_feat_test_ndoml1,cl_feature.HV.MS,"MS",0.05)
...
data11 <- na.omit(data.frame(mat.feat_func.onel2(full_feat_train_doml1,cl_feature.HV.MS,"HV",0.05)$pat_data[,c("F13","F24")], mat.feat_func.onel2(full_feat_train_doml1,cl_feature.HV.MS,"HV",0.05)$cl_data[,c("C20","C21")]))
data12 <- na.omit(data.frame(mat.feat_func.onel2(full_feat_train_doml1,cl_feature.HV.MS,"MS",0.05)$pat_data[,c("F13","F24")], mat.feat_func.onel2(full_feat_train_doml1,cl_feature.HV.MS,"MS",0.05)$cl_data[,c("C20","C21")]))
...
Here we will select only a number of clinical features using high correlation test and also removing features that have all 0 in the Healthy cohorts (Feature 15, 16, 26, 27, and 28)
