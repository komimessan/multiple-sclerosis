...
wtest_func_onedl <- function(data, alpha, group_var, Feat_ac){
  ...
  sub_data <- data %>% 
    dplyr::select(diagnosis_group, grep(Feat_ac,names(data), value = TRUE))
...
mat.feat_func.onel <- function(pat_data,cl_data, diagnosis, alpha){
  ...
  pat_sub <- merge_data %>% 
    dplyr::select(diagnosis_group, intersect(names(dplyr::select(merge_data,starts_with("F"))), 
  ...
  return(list(r=r_pat_cl, P=p_pat_cl, n=n_pat_cl))
} 
