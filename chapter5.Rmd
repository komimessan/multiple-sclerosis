...
mat.feat_func.onel <- function(pat_data,cl_data, diagnosis, alpha){
  ...
  pat_sub <- merge_data %>% ungroup() %>% 
    dplyr::select(diagnosis_group, intersect(names(dplyr::select(merge_data %>% ungroup(),starts_with("F"))), 
                           wtest_func_onedl(pat_data, alpha, "diagnosis_group", "F"))) %>% 
    dplyr::filter(diagnosis_group==diagnosis) %>% 
    dplyr::select(-c(diagnosis_group))
  ...
  # r_pat_cl <- cor_mat$r[19:n,1:18]
  # p_pat_cl <- cor_mat$P[19:n,1:18]
  # n_pat_cl <- cor_mat$n[19:n,1:18]
  #  ## plot
