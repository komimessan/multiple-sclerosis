...
Error_x_y <- subset(ms_data_train, 
                    (difficulty_level == 1 & y < -1.4 & abs(x) > 0.7) |
                    (difficulty_level == 2 & y < -0.8 & abs(x) > 1.2) |
                    (difficulty_level == 3 & y < -0.8 & abs(x) > 1.2))
...
normXY_func <- function(data){
  ...
  data$x_sc <- rep(0, dim(data)[1])
  data$y_sc <- rep(0, dim(data)[1])
    ...
    for (tn in n.test) {
      ...
      for (dl in d.levels) {
        ...
        for (tr in tr.levels){
          ...
          for (j in 1:(dim_tr)) {
            if (tr.level$y[1] > tr.level$y[dim_tr]){
              tr.level[j, "x_sc"] <- tr.level$x[j] + norm.xo
              tr.level[j, "y_sc"] <- tr.level$y[j] + norm.yo
            } else if (tr.level$y[1] < tr.level$y[dim_tr]){
              tr.level[j, "x_sc"] <- tr.level$x[j] + norm.xi
              tr.level[j, "y_sc"] <- tr.level$y[j] + norm.yi
            } else {
              tr.level[j,"x_sc"] <- NA
              tr.level[j,"y_sc"] <- NA
            }
...
# ####
# ptm <- proc.time() ## Check how long it will run (takes 9 min to run)
#   
