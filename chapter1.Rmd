...
## generate leeft handed spiral with difficulty 1, 2, and 3
...
p1 <- subset(ms_data4, patient_id=="NIB112") ## select single patient ##NDS668, NDS670
...
p111 <- subset(p11, difficulty_level=="Difficulty Level 1") ## select single difficulty level (1)
p1111 <- subset(p111, trial_id == p111$trial_id[1]) ## select one trial_id
...
### velocity vs. pixels
ggplot(p11_sub, aes(x=pixel, y= v_i))+
  geom_point(size  = 2.0)+
  geom_line(size = 0.9) +
  facet_wrap(~difficulty_level, scales = "free", nrow = 3) +
...
### velocity vs. time
ggplot(p11_sub, aes(x=time, y= v_i))+
  geom_point(size  = 2.0)+
  geom_line(size = 0.9) +
  facet_wrap(~difficulty_level, scales = "free", nrow = 3) +
