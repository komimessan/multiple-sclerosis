...
outlier_func <- function(dataframe){
  ...
  data_with_id <- dataframe %>% mutate(id = row_number())
  outlier_index <- tsoutliers(data_with_id$y, lambda = "auto")$index
  
  if (length(outlier_index) == 0){
    new_data = data_with_id %>% select(-id)
  } else {
    new_data = data_with_id %>% dplyr::filter(id != outlier_index) %>% select(-id)
  }
  return(new_data)
}


## Remove the outlier by trail group 
