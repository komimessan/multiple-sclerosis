...
ms_data <- read.xlsx("spiralLH_Obscured.xlsx", colNames = TRUE)
...
ms_data.psd <- ms_data3 %>% 
   group_by(patient_id,ntest_date,difficulty_level) %>%
  summarise(v_psd.max = max(pwelch(v_i)$spec), 
            v_df = pwelch(v_i)$freq[which.max(pwelch(v_i)$spec)],
            rv_psd.max = max(pwelch(rv_i)$spec),
            rv_df = pwelch(rv_i)$freq[which.max(pwelch(rv_i)$spec)],
            av_psd.max = max(pwelch(av_i)$spec),
            av_df = pwelch(av_i)$freq[which.max(pwelch(av_i)$spec)])
