...
```{r  message=FALSE, warning=FALSE, include=FALSE}
## Obtain the dimension of the data frame
d = dim(ms_data2)
...
p1 <- subset(ms_data2, patient_id=="rep067") ## select single patient (rep067)
...
for (id in p.id) {
  patiend_data <- subset(ms_data2, patient_id==id) ## select a single patient
  n.test <- levels(factor(patient_data$test_numb))
  for (tn in n.test) {
    test_date <- subset(patiend_data, test_numb==tn) ## select one test date
    d.levels <- levels(factor(test_date$difficulty_level))
    for (dl in d.levels) {
      d.level <- subset(test_date, difficulty_level==dl) ## select single difficulty level 
      
      for (i in 1:(dim(d.level)[1] - 1)) {
        d.level[i+1,"v_i"] <- sqrt((d.level$x[i+1] - d.level$x[i])^2 + 
                                     (d.level$y[i+1] - d.level$y[i])^2
                                   )/(d.level$t[i+1]-d.level$t[i])
      }
      new.d.level <- rbind(new.d.level, d.level) ## figure out how to deal with new.d.level
    }
  }
