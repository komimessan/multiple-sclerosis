...
## add visit number based on patientID, remove some columns and change column names
ms_data2 <- ms_data %>% 
  group_by(metaData.ID) %>%
  mutate(test_numb = factor(testdate, labels = 1:length(unique(testdate)))) %>% 
  select(metaData.ID,x,y,time.point,p,metaData.DIFFICULTY,sumData.Num.turns,
         sumData.Line.width,testdate, test_numb) %>% 
  rename(patient_id=metaData.ID,t = time.point,difficulty_level=metaData.DIFFICULTY,
